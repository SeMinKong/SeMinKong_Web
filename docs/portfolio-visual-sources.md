...
## Tech Stack SVG sources — updated 2026-09-12

The asset library contains 24 transparent SVGs in `src/assets/tech-stack/`, reproducible with `scripts/portfolio/export_stack_icons.mjs`: twenty Simple Icons brand paths (CC0-1.0), three functional pictograms and the existing official llama.cpp mark. About displays 16 icon instances from 15 files; Isaac Sim and Isaac Lab reuse the NVIDIA identification mark with distinct visible names. C++ and Python remain the centered programming-language row. The 2026-09-12 vLLM and Jira additions use Simple Icons 16.29.0 paths and brand colors, without changing the existing 22 files. Their upstream references are the [vLLM media kit](https://github.com/vllm-project/media-kit) and [Atlassian logo library](https://atlassian.design/resources/logo-library/); brand trademarks remain with their owners. `sources.json` records the exact per-file versions, sources and hashes. NVIDIA/Ollama retain their previously exported geometry; llama.cpp remains byte-identical to `src/assets/llama-cpp-logo.svg`.

Web icons keep visible Isaac names; other names are accessible on native summary controls and can be revealed without JavaScript. All 16 show the owner's five-point self-assessment. The current nine-page PDF uses searchable stack names and matching ratings on page 2, not these icons. Resume omits the duplicate learning-stack section. Earlier project-stack icons remain in the asset library; original architecture sources are untouched.
...
The current PDF is 27 pages: introduction 1, learning stack 2, index 3, THING 4–8, AQIS 9–14, Briefit 15–17, MRI 18–20, Prompt 21–23, Alkkagi 24–26, contact 27. Original architecture images remain on pages 5/10/16/19/22/25. The five owner-returned RGBA PNGs retain all pixels, alpha, proportions and source hashes. All project introductions, source imagery and technical coverage are retained. The learning-stack revision changes only page 2; all other 26 pages are pixel-identical to the preceding public PDF at 108dpi.
