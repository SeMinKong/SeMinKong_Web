...
각 프로젝트 소개 바로 다음의 4·8·11·13·15·17쪽은 소스 기반 기술 해설 한 쪽으로 배정한다. 제목 아래에 입력 → 처리 → 출력 흐름, 왼쪽에 원본 구조도, 오른쪽에 처리 원리·설계 선택 세 단락을 둔다. 구조도 아래에는 본인/팀 범위와 결과 해석에 필요한 한계를 짧게 표시한다. 하단 링크는 구현 당시 commit의 파일·함수로 연결한다. 실제 계산·데이터 변환·상태 전환을 설명하며 총 18쪽과 소개 시작 순서는 유지한다.
