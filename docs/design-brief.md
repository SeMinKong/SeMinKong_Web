...
각 프로젝트 소개 바로 다음의 4·9·13·16·19·22쪽은 기존 소스 기반 기술 해설을 보존한다. 제목 아래에 입력 → 처리 → 출력 흐름, 왼쪽에 원본 구조도, 오른쪽에 처리 원리·설계 선택 세 단락을 둔다. 구조도 아래에는 본인/팀 범위와 결과 해석에 필요한 한계를 짧게 표시한다. 하단 링크는 구현 당시 commit의 파일·함수로 연결한다. 실제 계산·데이터 변환·상태 전환을 설명하며 소개 시작 순서는 유지한다. 이어지는 5·10·14·17·20·23쪽에는 처리 도식·수식·의사코드 중심의 기술 페이지를 한 쪽씩 추가한다. 새 도식은 검색 가능한 PDF 벡터로 작성하고 원본 코드의 조건·단위·소유 범위를 표시한다. 개념도와 축약 의사코드를 실행 결과나 새 실험 수치로 표현하지 않는다.
