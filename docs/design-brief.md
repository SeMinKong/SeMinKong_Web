...
정적 PDF는 기존 18쪽을 보존하고 기술 도식 6쪽을 추가한 24쪽으로 구성한다. 프로젝트 순서는 THING → AQIS → Briefit → MRI → Prompt → Alkkagi이며, 여섯 프로젝트 모두 소개 페이지로 시작한다. 소개는 제품 목적·본인 역할, 상세는 문제·선택·구현 동작을 설명한다. 반복된 학력·학습 관심사 페이지는 첫 소개와 온라인 이력서로 정리하고, AQIS 좌표 갱신·예외 처리는 한 쪽에 통합한다. 이미지 번호 안내·기술 목록 재진술·작은 구현 수치를 줄인다. THING의 기구 편차·제어 순서, AQIS의 장비 연결·집기 상태, Briefit의 긴 입력 요약·정보 손실과 MRI·Prompt·Alkkagi의 구체적인 구현 판단을 남긴다. 한계는 결과 해석에 필요한 위치에서 간결하게 밝히며 확인된 구현·후속 Mock 재현·실물 시연을 구분한다. 원본 구조도와 수상 자료는 보존한다.

각 프로젝트 소개 바로 다음의 4·9·13·16·19·22쪽은 기존 소스 기반 기술 해설을 보존한다. 제목 아래에 입력 → 처리 → 출력 흐름, 왼쪽에 원본 구조도, 오른쪽에 처리 원리·설계 선택 세 단락을 둔다. 구조도 아래에는 본인/팀 범위와 결과 해석에 필요한 한계를 짧게 표시한다. 하단 링크는 구현 당시 commit의 파일·함수로 연결한다. 이어지는 5·10·14·17·20·23쪽에는 프로젝트마다 세 가지 구현 방법을 설명한다. 각 행의 왼쪽은 해결할 문제·선택한 방법·적용 조건, 오른쪽은 계산식·활성 설정·실행 분기를 담는다. 단순한 처리 순서나 기술 이름보다 관절각→엔코더 변환, 제어 주기 분리, 중복 판정과 신선도, 토큰 제한, 라벨 표현, 대화 상태, 충격량 등 원리를 우선한다. 본문 10.2pt와 검색 가능한 벡터 수식을 사용하고 본인/팀 기여·단위·예외를 표시한다. 설정값과 식 계산 예시를 실측 성과로 표현하지 않는다.
