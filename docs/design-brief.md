...
각 프로젝트 소개 바로 다음의 4·9·15·18·21·24쪽은 기존 소스 기반 기술 해설을 보존한다. 이어지는 5·10·16·19·22·25쪽에는 관절 벡터·엔코더 보간 그래프·제어 시간축, 중복/집기 분기, 토큰 띠, 마스크/다각형, 상태 전이, 충돌 벡터를 배치한다. 도식의 위치·길이·색과 짧은 설명·검색 가능한 수식이 구현 방법을 함께 설명한다. 최소 글자 9pt, 주요 설명 9.5~10pt이며 본인/팀 기여·단위·예외와 고정 commit 소스를 표시한다. 설정값·계산 예시·개념도를 실측 성과로 표현하지 않는다.
