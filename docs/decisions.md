...
## 2026-09-07 — 아키텍처 파일 선전달, PDF 적용 보류

- 사용자의 후속 요청에 따라 네 신규 구조도를 큰 컬러 SVG 로고, 비대칭 배치, 중첩된 실행 환경으로 다듬는다. Devicon 원본 경로와 Simple Icons를 함께 쓰며 배경이 없는 SVG와 편집 가능한 Excalidraw를 제공한다.
- 사용자가 직접 배경을 제거한 후 파일을 다시 전달하기로 했다. 먼저 THING·AQIS 원본 PNG와 신규 네 구조도의 흰 배경 고해상도 PNG, SVG·Excalidraw를 묶어 전달한다. 실험한 자동 배경제거 파생본은 선택하지 않고 scratch에만 보관한다.
- 이번 후속 작업의 PDF 적용·공개 다운로드 교체·커밋·배포는 보류한다. 공개 PDF는 36829e8 배포본을 유지하며, 사용자가 최종 이미지를 보내면 당시 레이아웃 및 PDF·웹 검증 후 적용한다.

- 스타일 재요청으로 v2의 가로 띠 구도를 폐기하고 `output/pdf/Architecture-THING-AQIS-style/`에 별도 전달본 네 개를 만든다. 약 4:3 구도, 실제 Excalidraw 0.18.1·Virgil 손글씨·거친 경계/곡선 화살표, 크게 배치한 공식 컬러 로고로 THING·AQIS의 기술 콜라주 스타일을 따른다. 파일은 PNG·SVG·Excalidraw이며 배경은 흰색으로 유지한다. 이 전달본은 PDF builder/public에 연결하지 않고, 사용자의 배경 제거 후 적용 결정을 기다린다.

## 2026-09-07 — Home 관절형 마네킹 이스터에그

- Form: 기존 추상 오브젝트를 얼굴 없는 머리·가슴·골반·상완/전완·허벅지/정강이 좌우의 11개 소형 부품으로 교체한다. 종이색·먹색·주홍색 관절만 사용하고 로봇 얼굴, HUD, 네온, 안내 문구는 넣지 않는다.
- Puzzle: neck·waist·shoulder·elbow·hip·knee 포트는 plug/socket 극성과 거리·각도를 함께 검사한다. 좌우 팔다리는 같은 family 안에서 서로 바꿔 조립할 수 있고, 결합된 부분은 관절을 유지한 하나의 조립체로 이동·회전한다. 관절 근처에서 상대 부품 반대 방향으로 당길 때만 분리한다.
- Collision: 부품끼리와 화면/내비게이션 경계만 충돌한다. 이름·소개 문장·CTA는 물리 장애물로 만들지 않아 부품이 시각적으로 지나갈 수 있으며, 드래그 입력은 CTA 위에서도 이어진다. CTA 자체의 클릭과 키보드 접근은 유지한다.
- Completion: 10개 관절이 하나의 11부품 그래프를 만들면 자세를 정돈한 뒤 고개를 기울이고 오른팔을 드는 짧은 동작, 관절 파동, 머리 주변 12개 작은 파편과 약한 밝기 변화만 실행한다. 문구와 소리는 추가하지 않는다. 감소 모션·강제 색상에서는 애니메이션 없이 완성형 정적 마네킹을 표시한다.
- Runtime: Pixi/Matter 런타임은 Home 전용 동적 import를 유지한다. 최종 production chunk는 209.37 kB raw / 61.94 kB gzip이며, 이전 195.70 / 57.12 kB 대비 증가분은 관절 그래프·스냅·분리·완성 연출에 한정되고 초기 공통 번들에는 포함되지 않는다.
- Scope: 웹 Home과 관련 검증·문서만 배포한다. 정적 PDF, Drive 파일, 보류된 아키텍처 작업 원본과 `tmp/`는 변경·배포 범위에서 제외한다.

## 2026-09-07 — 사용자 배경 제거본을 정적 PDF에 적용

- 사용자가 반환한 AQIS·Briefit·MRI·Alkkagi·Prompt Generator PNG 다섯 개를 최종 그림으로 채택한다. `scripts/portfolio/assets/architecture/returned/`에 바이트 그대로 보관하고 색상·알파 채널·전체 화면비를 유지해 종이색 위에 배치한다. THING은 사용자 요청에 따라 기존 검은 배경 원본과 5쪽 배치를 유지한다.
