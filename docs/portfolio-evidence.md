...
## Tech Stack selection — 2026-09-08

The owner requested a personal learning stack instead of a repeated project stack. The [2026-08-14 requested list](history/decisions.md#2026-08-14--current-stack-content-refresh) identifies thirteen tools: ROS 2, Isaac Sim, Isaac Lab, C++, Python, FastAPI, PyTorch, Ollama, llama.cpp, LangChain, Ubuntu, Git and Docker. Current About questions support the concise topics of ROS/joint control, sim-to-real differences and local inference constraints. The PDF and web show these same tools under programming languages and four learning areas. Project technologies and personal contribution evidence remain on their project pages. No course completion, proficiency rating, simulation success or fresh measured result is inferred.

Docker is supported by the Prompt project's [Dockerfile](https://github.com/SeMinKong/ProjectPromptGenerator_LangGraph/blob/c6ad3522c0db1fa351c9e74e5f50aeac4dab7ef0/Dockerfile) and Kong's [Deploy Update](https://github.com/SeMinKong/ProjectPromptGenerator_LangGraph/commit/c6ad3522c0db1fa351c9e74e5f50aeac4dab7ef0). Describe this as container execution configuration, not production-scale operations. Git/Ubuntu are development environment tools. On 2026-09-08 the owner explicitly identified C++ and Python as their programming languages and requested a centered row in the PDF and web Tech Stack. This owner-provided language listing does not extend project-specific ownership claims. Isaac Sim/Lab and Ollama/llama.cpp are now visible in the main learning stack, with no new proficiency or performance claims.

## Editorial structure

A4 landscape, 27 pages: introduction (1); Tech Stack (2); index (3); THING (4–8); AQIS (9–14); Briefit (15–17); MRI (18–20); Prompt (21–23); Alkkagi (24–26); contact (27). Six technical plates are visualized on pages 6, 11, 17, 20, 23 and 26. AQIS includes RoboDK and SLAM/robot telemetry pages 13–14; its introduction covers all three project areas. The previous 26-page content and project introductions remain, with updated index page references. The owner requested this exact project order and an introduction page at the start of every project. The cover remains personal; education details stay available in the linked Resume. Publication follows the standing GitHub Pages authorization after PDF and website verification.
...
## Readability and web coverage — 2026-09-08

All 26 pages were rendered and reviewed. Project titles use 23pt, source architecture pages reserve 407pt for the original image and 328pt for 10.4pt explanations. Korean paragraphs keep words intact. Reusable center connectors have equal 6pt endpoint gaps; verification checks every recorded arrow segment against text with 1pt clearance. Red is reserved for selected results rather than all equations.

Briefit, MRI, Prompt and Alkkagi web cases retain the original introduction/media and add an original architecture image, three source-linked implementation sections and concise limitations. The pinned sources below govern both media. Briefit describes successful-save URL deduplication within one run; MRI uses the real `no_tumor` label; Prompt rollback is limited to RuntimeError/ValueError/OSError, with incomplete success remaining in_progress; Alkkagi's 0.8 total damping holds only without collisions, stopping or repositioning. No fresh performance measurements were added.
