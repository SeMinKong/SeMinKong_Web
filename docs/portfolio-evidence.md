...
The GitHub profile, current About/Resume and the individual contribution records below were cross-checked for the added stack page. The four primary domains use personally implemented motor scripts/ROS integration, MRI preprocessing/inference, Briefit 2025 KoBART scripts, FastAPI/WebSocket/asyncio/LangChain, and React/TypeScript/Node.js/Socket.io. Profile-level descriptions do not establish personal ownership of THING retargeting, Briefit clustering or LangGraph execution.

Docker is supported by the Prompt project's [Dockerfile](https://github.com/SeMinKong/ProjectPromptGenerator_LangGraph/blob/c6ad3522c0db1fa351c9e74e5f50aeac4dab7ef0/Dockerfile) and Kong's [Deploy Update](https://github.com/SeMinKong/ProjectPromptGenerator_LangGraph/commit/c6ad3522c0db1fa351c9e74e5f50aeac4dab7ef0). Describe this as container execution configuration, not production-scale operations. Git/Ubuntu are development environment tools. On 2026-09-08 the owner explicitly identified C++ and Python as their programming languages and requested a centered row in the PDF and web Tech Stack. This owner-provided language listing does not extend project-specific ownership claims. Other existing tool listings remain in the web's supplementary usage/learning area; Isaac Sim/Lab and Ollama/llama.cpp remain learning topics. No new proficiency scores or performance results are asserted.
