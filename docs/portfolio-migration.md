...
후속 편집에서 왼쪽은 이미지·설명·회고, 오른쪽은 구현·트러블슈팅으로 재배치했다. 프로젝트 설명은 이미지 아래에 유지하고 기간·팀·역할은 제목 아래 한 줄로, 기술 스택은 하단으로 옮겼다. 원문을 모두 보존하며 오른쪽 본문은 10.3pt로 키우고 소제목 위치는 실제 글 길이에 맞춘다. 원본 픽셀·기존 이미지 표시 영역·비율, 수상 트로피·자료 링크와 중앙축을 유지한다. 페이지 번호는 표시하지 않는다. 표지·구현 경험·마지막 자료 페이지는 이번 재배치에서 변경하지 않았다.

- 추가 요청으로 2쪽 기술 스택에 작성자 지정 별점 16개와 자기평가 안내를 반영했다. 프로젝트 6쪽의 새 배치와 원문은 유지한다.
- 버전: 2026.09.12, A4 가로 9쪽, 4,527,050 bytes.
- SHA-256: `5D004F8E4DFB71DCF5EA43F8BA4921363EE17D77EE249707B04BF1FE264D8E31`.
